...
public class ShopServiceImpl implements ShopService{
	...
	public void buyPro(User user,long pid,int pnum) {
		...
		try {
			...
			BigDecimal priceBD = pro.getPrice();
			BigDecimal pnumBD = new BigDecimal(pnum);
			BigDecimal sumPrice = pnumBD.multiply(priceBD);		
			// 减少商品数量
			pdi.reduceProductAffair(pid,pnum);
			// 添加订单信息			
			long oid = odi.addOrderAffair(new Order(user.getId(),user.getUname(),user.getAddress(),sumPrice));
			odi.addOrderDeteilAffair(new OrderDetail(oid,pid,pnum,sumPrice));
			// 提交事务
