...
public class ShopServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		System.out.println("旧的："+old_pid);
		String[] old_pids = null;
		if (old_pid != null) {
			old_pids = old_pid.split("-");
		}
		String new_pid = null;
		if (old_pids != null) {
			String[] distict = distict(old_pids, pids);
			new_pid = getString(distict);
		}else {
			// 购物车以前没有数据时
			new_pid = getString(pids);
		}
		// 将拼接好的pid放到Cookie中发送到浏览器保存
		if (new_pid != null) {
			System.out.println(new_pid);
			Cookie cookie = new Cookie("pid", new_pid);	
