...
public class OrderDaoImpl implements OrderDao{
	...
	public List<Order> queryOrder(long uid) throws SQLException {
		...
		String sql = "SELECT id,uid,uname,uaddress,create_time,money,status,type FROM amz_order WHERE uid=? ORDER BY create_time DESC";
		return qr.query(sql,new BeanListHandler<Order>(Order.class,new BasicRowProcessor(new GenerousBeanProcessor())),uid);
	}
	// 根据订单id查询订单详情
	public OrderDetail queryOrderDetailById(long oid) throws SQLException {
		QueryRunner qr = C3P0Util.getQueryRunner();
		String sql = "SELECT id,oid,pid,quantity,money FROM amz_order_detail WHERE oid=? ";
		return qr.query(sql,new BeanHandler<OrderDetail>(OrderDetail.class),oid);
	}
